...
#define PORT_NOTIFY_ISR_DEFINE  uint32_t PortISRNestingCounter = 0

static inline void PortNotifyISRBegin(void)
{
    PortISRNestingCounter ++;

    /* Perform your ISR next, followed by PortNotifyISREnd() */
}

static inline void PortNotifyISREnd(void)
{
    PortISRNestingCounter --;
}
...
{
    return PortISRNestingCounter;   /* 0 = Thread mode, otherwise we are in an ISR */
}
