...
void ssip_handler(bm_register_file_t *stacked_regs)
{
    ...
    puts("SSIP handler in supervisor mode entered.");
...
int main(void)
{
    ...
    // Initialize interrupt handling for supervisor software interrupt
    bm_interrupt_init(BM_PRIV_MODE_SUPERVISOR);
    bm_interrupt_set_handler(BM_INTERRUPT_SSIP, ssip_handler);
    bm_interrupt_enable_source(BM_PRIV_MODE_SUPERVISOR, BM_INTERRUPT_SSIP);

    // Configure to handle supervisor software interrupt in supervisor mode
    bm_interrupt_delegate_source(BM_PRIV_MODE_SUPERVISOR, BM_INTERRUPT_SSIP);
